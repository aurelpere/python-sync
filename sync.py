...
class Sync():
    ...
    def copyfilec(src, dst, file, password):
        ...
        cwd = os.getcwd()
        with py7zr.SevenZipFile(f'{dst}/{file}.7z', mode='w', password=password) as archive:
            os.chdir(src)
    ...
    def copyfiled(src, dst, file, password):
        ...
        with py7zr.SevenZipFile(f'{src}/{file}', mode='r', password=password) as archive:
    ...
    def cypherfolders(self,source, dest,password=''):
        ...
        for subfolder in folderlist:
            if os.path.isdir(f'{dest}/{subfolder}'):
                print(f"{dest}/{subfolder} existe ou il y a eu un message d'erreur de copie")
            else:
                self.mkdirs(f'{dest}/{subfolder}')
    ...
    def decypherfolders(self,source, dest,password=''):
        ...
        if they dont exist or are more recent"""
        if self.keyword!='':
            fileslist = self.listfileskw(source,r'({}).+(\.7z$)'.format(self.keyword))
        else:
        ...
        for subfolder in folderlist:
            ...
            self.decypherfolders(f'{source}/{subfolder}', f'{dest}/{subfolder}')
    @staticmethod
    ...
    def cypher(self,password=''):
        ...
        self.cypherfolders(self.sourcefolder, self.destfolder, password=password)

    def decypher(self,password=''):
        """cypher method made for interpreter use :
        imported_module.Sync(sourcefolder,destfolder,keyword).decypher('password')"""
        self.decypherfolders(self.sourcefolder, self.destfolder, password=password)
...
if __name__ == "__main__":
    ...
    sync_object=Sync(arguments['origin'],arguments['dest'],keyword=arguments['keyword'])
    sync_object.choose(cypherpw=arguments['cypher'],decypherpw=arguments['decypher'])
