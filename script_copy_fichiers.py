...
def dechiffrer(source,dest):
    ...
    for i in a:
        ...
        if os.path.isfile(str(dest)+"/"+z): #si le fichier dest existe sans .7z
            ...
            if timestampsrc[8]>timestampdest[8]: #si le timestamp source est plus récent que celui du dest
                prgmrun="7z e "+str(source)+"/"+"\""+i+"\""+" -pmdp -o"+str(dest)
                os.system(prgmrun) 
    ...
    for j in b:
        if os.path.isdir(str(dest)+'/'+j):
            print(str(dest)+"/"+j+" existe ou il y a eu un message d'erreur de copie")            
        else:
            os.mkdir(str(dest)+'/'+j)    
...
def recopier(source,dest):
    ...
    for i in a:
        if os.path.isfile(str(dest)+'/'+i): #si le fichier dest existe
            if not (filecmp.cmp(str(i),str(dest)+'/'+str(i))): #si le fichier dest est différent du fichier source
                timestampdest=os.stat(str(dest)+'/'+i)          
                timestampsrc=os.stat(str(source)+'/'+i)
                if timestampsrc[8]>timestampdest[8]: #si le fichier source est plus récent que le fichier dest
...
def chiffrer(source,dest):
    ...
    #print(a)    
    for i in a:
        if os.path.isfile(str(dest)+"/"+i+".7z"): #si le fichier dest existe en .7z
            timestampdest=os.stat(str(dest)+"/"+i+".7z")          
            timestampsrc=os.stat(str(source)+"/"+i)
            if timestampsrc[8]>timestampdest[8]: #si le timestamp source est plus récent que celui du dest
                prgmrun="7z a -mhe=on -pmdp "+"\""+str(dest)+"/"+str(i)+".7z"+"\""+" \""+str(i)+"\""+" -scrc SHA256 -stl" 
...
timedebutaller=[time.localtime()[2],'0'+str(time.localtime()[1]),time.localtime()[0],str(time.localtime()[3])+'h',str(time.localtime()[4])+'mn']
...
timefinaller=[time.localtime()[2],'0'+str(time.localtime()[1]),time.localtime()[0],str(time.localtime()[3])+'h',str(time.localtime()[4])+'mn']
print('enregistrement heure fin copie aller')
print('copie didf')
timefinretour=[time.localtime()[2],'0'+str(time.localtime()[1]),time.localtime()[0],str(time.localtime()[3])+'h',str(time.localtime()[4])+'mn']
